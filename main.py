...
option = ["Dogecoin_click_bot", "Litecoin_click_bot", "BCH_clickbot",
          "Zcash_click_bot", "Bitcoinclick_bot"]  # Bot option list
...
ask = int(input("	\n	Which bot do you want to run?" +
          Fore.RED + " (Number only)" + Fore.RESET + ":"))
answer = (option[ask])
url_channel = answer
...
async def main():
    ...
    for number, letter in enumerate(action):
        print(	"	", number, letter)
    # Ask user to select bot
    ask_action = int(input("	\n	What bot's action do you want to perform?" +
                     Fore.RED + " (Number only)" + Fore.RESET + ":"))
    ...
    if answer == action[0]:
            ...
            if type(original_update) is not UpdateShortMessage:
                if hasattr(original_update.message, 'reply_markup') and type(original_update.message.reply_markup) is ReplyInlineMarkup:
                    ...
                    if url is not None:
                        ...
                        captcha = parse_data.find(
                            'div', {'class': 'g-recaptcha'})

                        # Captcha detected
                        if captcha is not None:
                            print_msg_time(
                                Fore.RED + 'Captcha detected!' + Fore.RED + ' Skipping ads...\n')
        ...
        async def no_valid(event):
            ...
            if 'Sorry, that task is no longer valid' in message:
                print_msg_time(
                    Fore.RED + 'Sorry, that task is no longer valid.' + Fore.RESET)
                print_msg_time(
                    Fore.YELLOW + 'Sending /visit command' + Fore.RESET)
                await client.send_message(url_channel, '/visit')
        # No more ads
        ...
        async def no_ads(event):
            ...
            if 'no new ads available' in message:
                print_msg_time(
                    Fore.RED + 'Sorry, there are no new ads available\n' + Fore.RESET)
                e = input('Press any key to exit...')
                exit(1)
        ...
        async def join_start(event):
            ...
            if 'You must join' in message:
                channel_name = re.search(
                    r'You must join @(.*?) to earn', message).group(1)
        ...
        async def wait_hours(event):
            ...
            if 'You must stay' in message:
                waiting_hours = re.search(
                    r'at least (.*?) to earn', message).group(1)
                print_msg_time(
                    Fore.GREEN + f'Success! Please wait {waiting_hours} to earn reward\n' + Fore.RESET)
